...
namespace VideoGameApp.Services
{
    public class VideoGameService  // ✅ Fixed Typo Here
    {
        ...
        {
            return await _context.VideoGames.FirstOrDefaultAsync(vg => vg.Id == id);
        }
        ...
        public async Task AddVideoGameAsync(VideoGame videoGame)
        {
            ...
            if (await _context.VideoGames.AnyAsync(vg => vg.Title.Equals(videoGame.Title, StringComparison.OrdinalIgnoreCase)))
            {
        ...
        public async Task UpdateVideoGameAsync(VideoGame videoGame)
        {
            ...
            var existingVideoGame = await _context.VideoGames.FirstOrDefaultAsync(vg => vg.Id == videoGame.Id);
        ...
        {
            var videoGame = await _context.VideoGames.FirstOrDefaultAsync(vg => vg.Id == id);
            if (videoGame != null)
            {
